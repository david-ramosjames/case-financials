-- Case Expenses (vendor/case costs filed to Dropbox Expenses folder)
-- Run in the CLIENT Supabase project (after 001–004).
...
create index if not exists idx_case_expenses_case_number
  on public.case_expenses(case_number);

create index if not exists idx_case_expenses_case_id
  on public.case_expenses(case_id)
  where case_id is not null;

create index if not exists idx_case_expenses_review_status
  on public.case_expenses(review_status);

create index if not exists idx_case_expenses_created_at
  on public.case_expenses(created_at desc);

create index if not exists idx_case_expenses_dropbox_file_id
  on public.case_expenses(dropbox_file_id)
  where dropbox_file_id is not null;

comment on table public.case_expenses is
  'Non-medical case costs (vendor invoices, investigation, filing fees, etc.) from Dropbox Expenses folder.';
